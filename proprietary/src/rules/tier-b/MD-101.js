...
exports.rule = void 0;
const license_gate_js_1 = require("../../license-gate.js");
const policy_manifest_js_1 = require("./policy-manifest.js");
// ─── Gap Detection ───────────────────────────────────────────────────
function repoShortName(fullName) {
    return fullName.includes('/') ? fullName.split('/')[1] : fullName;
}
function makeGapFinding(repo, expectation, detail) {
    ...
    return {
        ...
        remediation: `Add the required ${expectation.category} configuration to "${fullName}". ` +
...
function checkBranchProtection(repo, expectation, req) {
    ...
    if (req.requiredReviewers !== undefined &&
        ...
        issues.push(`${repo.branchProtection.requiredReviewers} reviewers, need ${req.requiredReviewers}`);
    }
...
function checkStatusChecks(repo, expectation, req) {
    if (!repo.branchProtection) {
        return [makeGapFinding(repo, expectation, `no branch protection — cannot enforce status checks`)];
    }
    ...
    if (missing.length > 0) {
        return [makeGapFinding(repo, expectation, `missing ${missing.join(', ')}`)];
    }
    if (req.requireStrictStatusChecks && !repo.branchProtection.requireStrictStatusChecks) {
        return [makeGapFinding(repo, expectation, 'strict status checks not required')];
    }
    return [];
}
function checkWorkflowPresence(repo, expectation, req) {
    let match = repo.workflows.find(w => w.path === req.workflowFile);
    if (!match && req.workflowPattern) {
        match = repo.workflows.find(w => (0, policy_manifest_js_1.matchGlob)(req.workflowPattern, w.path));
    }
    if (!match) {
    ...
    if (req.requiredJobs && req.requiredJobs.length > 0) {
        ...
        if (missingJobs.length > 0) {
            return [makeGapFinding(repo, expectation, `missing required jobs: ${missingJobs.join(', ')}`)];
        }
    }
    return [];
}
function checkPermissions(repo, expectation, req) {
    if (req.maxDefaultPermissions === 'read' && repo.defaultPermissions === 'write') {
        return [makeGapFinding(repo, expectation, 'default permissions are "write", policy requires "read"')];
    }
    return [];
}
function checkCodeowners(repo, expectation, req) {
    const covered = repo.codeowners.coveredPaths;
    const missingPaths = req.requiredPaths.filter(reqPath => !covered.some(covPath => covPath.startsWith(reqPath) || reqPath.startsWith(covPath)));
    if (missingPaths.length > 0) {
        return [makeGapFinding(repo, expectation, `CODEOWNERS missing coverage for ${missingPaths.join(', ')}`)];
    }
    return [];
}
function checkExpectation(repo, expectation) {
...
exports.rule = {
    ...
    description: 'Detects protection asymmetry across repositories in the same organization. ' +
    ...
    evaluate: async (context) => {
        (0, license_gate_js_1.requirePro)(context, 'MD-101: Cross-Repo Protection Gap');
        const findings = [];
        const orgContext = context.orgContext;
        ...
        if (!orgContext) {
            findings.push({
                ...
                description: 'Cross-repo protection gap analysis requires an OrgContext with a policy ' +
                ...
                remediation: 'Provide an orgContext object containing a policy manifest and repo states. ' +
        ...
        // ── Validate manifest ──
        const validation = (0, policy_manifest_js_1.validateManifest)(orgContext.manifest);
        if (!validation.valid) {
            findings.push({
                ...
                description: `The organization policy manifest has ${validation.errors.length} error(s): ` +
                ...
                remediation: 'Fix the errors in the policy manifest before cross-repo governance ' +
        ...
        for (const repo of orgContext.repos) {
                ...
                if (exemption.repo === shortName && exemption.expiresAt) {
                    ...
                    if (expiry < now) {
                        findings.push({
                            ...
                            description: `The exemption for "${repo.fullName}" covering expectations ` +
                            ...
                            remediation: `Either bring "${repo.fullName}" into compliance with the exempted ` +
            ...
            const { expectations } = (0, policy_manifest_js_1.resolveExpectationsForRepo)(orgContext.manifest, shortName, {
...
//# sourceMappingURL=MD-101.js.map
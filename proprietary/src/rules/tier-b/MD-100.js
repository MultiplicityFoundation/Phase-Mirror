...
exports.rule = exports.INTENT_VOCABULARY = void 0;
exports.parseWorkflowJobs = parseWorkflowJobs;
exports.detectJobIntent = detectJobIntent;
exports.stepsMatchIntent = stepsMatchIntent;
exports.detectEnvironmentDrift = detectEnvironmentDrift;
const license_gate_js_1 = require("../../license-gate.js");
exports.INTENT_VOCABULARY = [
    {
...
function parseWorkflowJobs(content, filePath) {
    const jobs = [];
...
function detectJobIntent(jobKey, jobName) {
    const nameToCheck = `${jobKey} ${jobName ?? ''}`.toLowerCase();
    const matches = [];
    for (const signal of exports.INTENT_VOCABULARY) {
...
function stepsMatchIntent(steps, intents) {
    const matchedCategories = [];
    const unmatchedCategories = [];
    for (const intent of intents) {
        const signal = exports.INTENT_VOCABULARY.find(s => s.keywords[0] === intent.category);
        if (!signal)
            continue;
        const hasMatchingAction = steps.some(step => step.uses && signal.expectedActions.some(action => step.uses.toLowerCase().includes(action.toLowerCase())));
        const hasMatchingCommand = steps.some(step => step.run && signal.expectedCommands.some(cmd => step.run.toLowerCase().includes(cmd.toLowerCase())));
...
function detectEnvironmentDrift(job) {
    ...
    if (!claimsStaging && !claimsProduction)
        return null;
    for (const step of job.steps) {
        const commandText = `${step.run ?? ''} ${step.uses ?? ''} ${step.name ?? ''}`.toLowerCase();
        if (claimsStaging && (commandText.includes('production') ||
            ...
            return {
                ...
                description: `The job "${job.jobKey}" has a name suggesting staging/dev deployment, ` +
                ...
                remediation: 'Rename the job to reflect its actual target environment, or ' +
...
exports.rule = {
    ...
    description: 'Detects when CI/CD job names suggest one purpose but the actual steps ' +
        'perform a different function. This is a governance risk because branch ' +
        'protection rules reference job names — if the name lies, the protection lies.',
    version: '1.0.0',
    tier: 'B',
    ...
    evaluate: async (context) => {
        (0, license_gate_js_1.requirePro)(context, 'MD-100: Semantic Job Drift');
        const findings = [];
        // Filter for workflow files
        const workflowFiles = (context.files ?? []).filter(f => f.path.includes('.github/workflows/') &&
        ...
        for (const file of workflowFiles) {
            ...
            for (const job of jobs) {
                ...
                const nonCheckoutSteps = job.steps.filter(s => !(s.uses && s.uses.startsWith('actions/checkout')));
                if (nonCheckoutSteps.length === 0)
                    continue;
                // Detect declared intent from job name
                const intents = detectJobIntent(job.jobKey, job.jobName);
                if (intents.length === 0)
                    continue; // No recognizable intent — skip
                ...
                if (!match.matched && match.unmatchedCategories.length > 0) {
                    ...
                    findings.push({
                        ...
                        description: `The job "${job.jobKey}"${job.jobName ? ` ("${job.jobName}")` : ''} ` +
                        ...
                        remediation: `Either rename the job to reflect what it actually does, or add ` +
...
//# sourceMappingURL=MD-100.js.map
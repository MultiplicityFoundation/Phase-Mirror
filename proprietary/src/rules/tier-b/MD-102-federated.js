...
exports.evaluateMD102Federated = evaluateMD102Federated;
 ...
 */
async function evaluateMD102Federated(ctx) {
    const findings = [];
...
function evaluateRepoAgainstOrgPolicy(repo, bp, policy) {
    const findings = [];
    ...
    if (policy.requiredForDefaultBranch && (!bp || !bp.requirePullRequest)) {
        findings.push({
            ...
            description: `Branch protection for ${fullName}/${repo.meta.defaultBranch} does not require ` +
            ...
            remediation: `Enable "Require a pull request before merging" for ${fullName}/${repo.meta.defaultBranch}, ` +
    ...
    if (repo.meta.tags?.includes('critical') && !repo.mergeQueue?.enabled) {
        findings.push({
            ...
            description: `Repository ${fullName} is tagged "critical" but does not have a merge queue enabled. ` +
            ...
            remediation: `Enable the merge queue on ${fullName}, or remove the "critical" tag and update ` +
    ...
    if (!policy.allowBypassForAdmins && bp?.enforceAdmins === false) {
        findings.push({
            ...
            description: `The org merge queue policy forbids admin bypass, but ${fullName} does not enforce ` +
            ...
            remediation: `Enable "Do not allow bypassing the above settings" (enforceAdmins) on ${fullName}, ` +
...
//# sourceMappingURL=MD-102-federated.js.map
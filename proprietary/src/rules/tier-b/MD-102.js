...
exports.rule = {
    ...
    description: 'Detects inconsistent branch protection and status check configuration ' +
    ...
    evaluate: async (context) => {
        (0, license_gate_js_1.requirePro)(context, 'MD-102: Merge Queue Trust Chain Break');
        const findings = [];
        // Extract extended context fields
        const branchProtection = context.branchProtection;
        const workflowJobs = context.workflowJobs;
        const mergeQueuePolicy = context.mergeQueuePolicy;
        ...
        if (!branchProtection) {
            if (mergeQueuePolicy.requiredForDefaultBranch) {
                findings.push({
                    ...
                    description: 'The merge queue policy declares that branch protection is required for the ' +
                    ...
                    remediation: 'Enable branch protection on the default branch and configure the merge queue, ' +
        ...
        if (!mergeQueuePolicy.allowBypassForAdmins && branchProtection.allowAdminsBypass) {
            findings.push({
                ...
                description: 'The branch protection allows administrators to bypass all protection rules, ' +
                ...
                remediation: 'Disable "Allow administrators to bypass branch protection rules" for the default branch, ' +
        ...
        if (!mergeQueuePolicy.allowDirectPushes &&
            ...
            findings.push({
                ...
                description: 'Direct pushes to the default branch are possible (either explicitly allowed ' +
                ...
                remediation: 'Enable "Require a pull request before merging" and disable "Allow direct pushes" ' +
        ...
        if (mergeQueuePolicy.requireLinearHistory && !branchProtection.requireLinearHistory) {
            findings.push({
                ...
                description: 'The merge queue policy requires linear history (rebase or squash merges only), ' +
                ...
                remediation: 'Enable "Require linear history" in branch protection settings, or relax the ' +
        ...
        if (workflowJobs && branchProtection.requiredStatusChecks) {
            ...
            if (missingContexts.length > 0) {
                findings.push({
                    ...
                    description: `Required status checks [${missingContexts.join(', ')}] are not produced by ` +
                    ...
                    remediation: 'Align branch protection required checks to job contexts that exist in .github/workflows/, ' +
...
//# sourceMappingURL=MD-102.js.map
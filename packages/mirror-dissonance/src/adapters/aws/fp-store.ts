...
export class FPStoreError extends Error {
  constructor(
    ...
    super(`FPStore ${operation} failed for ${resourceId}`);
...
export class AWSFPStore implements FPStoreAdapter {
  ...
  async recordEvent(event: FPEvent): Promise<void> {
    ...
    try {
      ...
      throw new FPStoreError("recordEvent", event.eventId, error);
    }
  }
  ...
  async markFalsePositive(
    ...
    if (!queryResult.Items || queryResult.Items.length === 0) {
      throw new FPStoreError(
        "markFalsePositive",
        findingId,
        new Error(`Finding ${findingId} not found in FP store`)
      );
    }
  ...
  async getWindowByCount(ruleId: string, count: number): Promise<FPWindow> {
    try {
      ...
      throw new FPStoreError("getWindowByCount", ruleId, error);
    }
  }
  ...
  async getWindowBySince(ruleId: string, since: Date): Promise<FPWindow> {
    try {
      ...
      throw new FPStoreError("getWindowBySince", ruleId, error);
    }
  }
  ...
  private unmarshallEvent(raw: Record<string, any>): FPEvent {
    return {
      ...
      isFalsePositive: raw.isFalsePositive || false,
